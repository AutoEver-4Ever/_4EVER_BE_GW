...
public class AlarmSendServiceImpl implements AlarmSendService {
    ...
    @Override
    public void sendAlarmMessage(String userId, String message, Object data) {
        log.info("알림 메시지 전송 시작 - UserId: {}, Message: {}", userId, message);
        ...
        try {
            ...
            emitter.send(event);
            log.info("알림 메시지 전송 성공 - UserId: {}", userId);
            
        } catch (IOException e) {
            log.error("알림 메시지 전송 실패 - UserId: {}", userId, e);
            emitterMap.remove(userId);
            try {
                emitter.completeWithError(e);
            } catch (Exception ex) {
                log.warn("SSE Emitter 에러 처리 중 오류 - UserId: {}", userId, ex);
            }
        }
    }
}
